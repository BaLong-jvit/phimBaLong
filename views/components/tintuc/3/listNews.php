<!-- listInNews.php tin tuc trong nuoc -->
<?php 
$page=(isset($_REQUEST['page']))?$_REQUEST['page']:1;
	...
	$list_news=$listNew->get_list_news($parent_id,$page);
	...
	if ($total_page<2) {
		...
		if ($page==1) {
			...
			$btn_pageHTML.="<li class='btn_page ''>
				<a href='?option=".$option."&local=".$local."&list=".$list."&page=".($i+1)."' class='page_button";
				if(($i+1)==$page){
			...
			$btn_pageHTML.="<li class='btn_page next'>
				<a href='?option=".$option."&local=".$local."&list=".$list."&page=".($page+1)."' class='next_button'>Next</a>
			</li>";
		}elseif ($page==$total_page) {
			$btn_pageHTML.="<li class='btn_page next'>
					<a href='?option=".$option."&local=".$local."&list=".$list."&page=".($page-1)."' class='pre_button'>Previous</a>
				</li>";
			for ($i=0; $i < $total_page; $i++) { 
				$btn_pageHTML.="<li class='btn_page ''>
				<a href='?option=".$option."&local=".$local."&list=".$list."&page=".($i+1)."' class='page_button";
	...
	$str_list_short_news="";
	foreach ($list_news as $item_short_news) {
		...
		$str_list_short_news.="			<a href='?option=tintuc&id-news=".$item_short_news['id']."' class='detail-news'>Xem chi tiết &gt;</a>";
		$str_list_short_news.="		</div>";
		$str_list_short_news.="	</div>";
		$str_list_short_news.="</li>";
	}
	$noidung=loadModel('ConnectDB');
	$label=$noidung->get_content_page($parent_id);

?>
<div class="wapper-list-news">
	<h5>Tin trong nước</h5>
